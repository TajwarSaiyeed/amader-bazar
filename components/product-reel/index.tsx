...
const ProductReel = (props: ProductReelProps) => {
    const {title, subtitle, href, data, loading = false, skeletonNumber = 4} = props;
        ...
        <section className={'py-12'}>
            <div className={'md:flex md:items-center md:justify-between mb-4'}>
                <div className={'max-w-2xl px-4 lg:max-w-4xl lg:px-0'}>
                    {title ? <h1 className={'text-2xl font-bold text-gray-900 sm:text-3xl dark:text-gray-200'}>
                        {title}
                    </h1> : null}
                ...
                {href ?
                    <Link href={href}
                        ...
                        Shop the collection <span aria-hidden={true}>
                        &rarr;
                    </span>
                    </Link> : null}
            </div>
            <div className={'relative'}>
                <div className={'mt-6 flex items-center w-full'}>
                    <div
                        ...
                        {products.map((product, i) => <ProductListing key={i} index={i} product={product}/>)}
                    </div>
                </div>
            </div>
        </section>
